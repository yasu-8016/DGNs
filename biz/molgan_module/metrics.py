...
"""
This biz is retrieved from:
https://github.com/nicola-decao/MolGAN
 
"""


NP_model = pickle.load(gzip.open('biz/molgan_module/data/NP_score.pkl.gz'))
SA_model = {i[j]: float(i[0])
            for i in pickle.load(gzip.open('biz/molgan_module/data/SA_score.pkl.gz'))
...
def main():
    mol_ = [Chem.MolFromSmiles('CC')]

    mm = MolecularMetrics()

    print(mm.valid_filter(mols=mol_))
    print(mm.synthetic_accessibility_score_scores(mols=mol_))
    print(mm.drugcandidate_scores(mols=mol_))
